    ...
    try {
      ...
      if (oldChatRoom.Item.nameGroup !== nameGroup) {
        const systemMessage = await createSystemMessage(
          roomId,
          participants,
          `${phone} đã đổi tên nhóm thành ${nameGroup}`,
          "CHANGE_NAME"
        );
      ...
      if (addedMembers.length > 0) {
        const systemMessage = await createSystemMessage(
          roomId,
          participants,
          `${phone} đã thêm ${addedMembers.join(", ")} vào nhóm`,
          "ADD_MEMBER"
        );
        io.to(roomId).emit("receiveMessage", systemMessage);
      }
      if (removedMembers.length > 0) {
        const systemMessage = await createSystemMessage(
          roomId,
          participants,
          `${phone} đã xóa ${removedMembers.join(", ")} khỏi nhóm`,
          "REMOVE_MEMBER"
        );
        io.to(roomId).emit("receiveMessage", systemMessage);
      }
  ...
  router.put("/removeMember", async (req, res) => {
    ...
    try {
      ...
      const systemMessage = await createSystemMessage(
        chatRoomId,
        updatedParticipants,
        `${phoneNumber} đã bị xóa khỏi nhóm`,
        "REMOVE_MEMBER"
      );
  ...
  router.post("/setAdmin", async (req, res) => {
    ...
    try {
      ...
      const systemMessage = await createSystemMessage(
        ...
        `${phoneNumber} đã được chỉ định làm quản trị viên`,
        "SET_ADMIN"
      );
      io.to(chatRoomId).emit("receiveMessage", systemMessage);
